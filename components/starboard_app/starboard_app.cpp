...
void AppManager::gotoApp(AppBase *app)
{
    if (!app)
        return;
    if (app == current) { pendingSwitch = nullptr; pendingBack = false; return; }
    pendingSwitch = app;
    pendingBack = false;
}
...
void AppManager::run()
{
    ...
    // 3b. Lua App 退出(requestSelector)→ 进 App 列表;选中则链式切到目标 App
    if (pendingSelector)
    {
        pendingSelector = false;
        openSelector(); // 内部 GUI::menu 阻塞;选中设 pendingSwitch
        if (pendingSwitch || pendingBack) goto step3; // 链式切换,同保持期模式
    }
