...
- 2026-06-28 —— **Lua App 休眠根治方案制定**（详见 [`LUA_APP_SLEEP.md`](./LUA_APP_SLEEP.md)）。诊断 Web IDE 生成的 Lua App 不能被系统休眠接管的三大病灶（① 控制流割裂：setup 阻塞 ulTaskNotifyTake 导致保持期永不运行；② 30s 硬超时后后台 Lua 任务泄漏、与 display 竞态；③ requestLuaStop 基础设施已有但未接系统侧）。关键约束：主任务栈仅 3584（`CONFIG_ESP_MAIN_TASK_STACK_SIZE`）→ 不能挪 Lua 到主任务，保留专用任务（8192 栈）；Task WDT 只监控 IDLE，hook 每 2000 行 vTaskDelay(1) 喂狗够。定方案 A：把「无操作超时→深睡 + 中键长按>1s→退出」做进 LINE hook 与 delay/waitKey 等 yield 点，Lua 任务自停后通知 setup() 返回，控制流回归单线程、无竞态无泄漏。经 Plan agent 审查锁定三致命点（删任务内 goBack、appManager 新增 pendingSelector 连线、menu/msgbox 的 C 阻塞盲区必须本轮用 GUI::setStopCheck 哨兵修）。用户确认中键长按>1 秒为系统保留退出手势。**已实现（见下条）**。
- 2026-06-28 —— **Lua App 休眠根治代码实现完成**（7 步全落地，待 `idf.py build` + 烧录验证）。① `starboard_gui` 加 `setStopCheck`/`resetStopCheck` + `waitKeyEvent` 返回 `KEY_STOP` 哨兵，menu/msgbox/msgbox_yn/msgbox_number/msgbox_time 识别哨兵提前返回；② `starboard_lua` 加 `luaSysPollStop`/`luaSysTick`/`luaSysBeginRun`/`luaSysEndRun`/`luaSysStopReason`（活动刷新 + 无操作超时 + 中键长按>1s 边沿计时），hook/delay/waitKey/tryGetKey/sys.yield 全接入；③ `lua_app_wrapper` 重写 `luaAppTask`（删任务内 goBack）+ setup 据 `luaSysStopReason` 路由（EXIT/NONE→requestSelector、SLEEP→保持期深睡），去 30s 硬超时；④ `starboard_app` 加 `pendingSelector`/`requestSelector` + run() 连线（goto step3）；⑤ `lua_gui` menu/msgbox 绑定补 `luaL_error`；⑥ `lua_webserver` 的 `pollRunRequest` 豁免超时深睡 + 删 `killMonitorTask`（hook 取代）；⑦ `lua_http` timeout 10s→3s。修 C linkage（`luaSysPollStopCpp` 包装）、补 lua 组件 REQUIRES starboard_config。详见 [`LUA_APP_SLEEP.md`](./LUA_APP_SLEEP.md) §4/§7/§8。
